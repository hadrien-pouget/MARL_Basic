...
class BinCommunicationGame(IncompleteInfoGame):
    ...
    def gen_rand_policies(self, device='cpu'):
        return torch.randn(10, requires_grad=True, device=device), torch.randn(12, requires_grad=True, device=device)
    ...
    def plot_with_equilibria(self, folder, prior_name=None):
        prior_name = 'Player1' if prior_name is None else prior_name
        dic = QuickSaver().load_json_path(os.path.join('quick_saves', folder, prior_name+'_Pfs_0.json'))
        r1s = [x[0] for k, x in dic.items()]
        r2s = [x[1] for k, x in dic.items()]
        dic = QuickSaver().load_json_path(os.path.join('quick_saves', folder, 'Pols_0.json'))
        p1s = [x[0] for k, x in dic.items()]
        p2s = [x[1] for k, x in dic.items()]
...
class SimpleDistInfComms(BinCommunicationGame):
    def __init__(self, a, prior_1_param, prior_2_param):
            ...
            with communication.
            prior_[1,2] is the probability that they both prefer the same the outcome,
...
class DistInfComms(BinCommunicationGame):
    def __init__(self, p, a):
        ...
        super().__init__(games, prior_1=prior, prior_2=prior)
...
def game_value(payoffs, p1, p2, device):
    """
...
def get_value_comm1(payoffs, p1, p2, ts, s1, device):
    """
    ...
    for s2 in [0, 1]:
        v1, v2 = game_value(payoffs, get_param_p1(p1, ts[0], s1, s2), get_param_p2(p2, ts[1], s1, s2), device)
...
def get_value_bincomms_oneshot(payoffs, p1, p2, ts, device):
    """
    Value given type
    """
    vs_per_game_1 = []
    ...
    for s1 in [0, 1]:
        v1, v2 = get_value_comm1(payoffs, p1, p2, ts, s1, device)
        vs_per_game_1.append(v1)
        vs_per_game_2.append(v2)
...
def get_value_incomplete_bincomms_oneshot(payoffs, p1, p2, dist):
    ...
    for t1, t2 in [(0, 0), (0, 1), (1, 0), (1, 1)]:
        v1, v2 = get_value_bincomms_oneshot(payoffs[t1][t2], p1, p2, (t1, t2), device)
        vs_per_game_1.append(v1)
        vs_per_game_2.append(v2)
