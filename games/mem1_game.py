...
class Mem1Game(IncompleteInfoGame):
    ...
    """
    def __init__(self, payoffs, prior=None, prior_n=0, oneshot=False):
        super().__init__(payoffs, prior=prior, prior_n=prior_n)
    ...
    def get_value(self, p1, p2, prior=None, gamma=0.96, **kwargs):
        if prior is None:
            prior = self.prior
        if self.oneshot:
...
class IncompleteFour(Mem1Game):
    def __init__(self, prior_n=0, oneshot=False):
        super().__init__(four_games, prior_n=prior_n, oneshot=oneshot)
...
class DistInf(Mem1Game):
    def __init__(self, p, a, oneshot=False):
        ...
        super().__init__(games, prior=prior, oneshot=oneshot)
        self.name = "DistInf"
